...
export default function MetaPixel() {
  /* Two captures, both on first landing, both on EVERY page rather than only
     ...
     onto /register, and that visit is the only one carrying the campaign.
     ...
     what the registration record in Pabbly is built from.

     THIS RUNS ABOVE THE PIXEL_ID GUARD ON PURPOSE. Attribution must not go
     dark just because a pixel id is unset: an unset pixel is an analytics
     problem, a blank campaign field is a sale that reports as organic forever. */
  useEffect(() => {
    captureFbclid();
    captureAttribution();
  }, []);
