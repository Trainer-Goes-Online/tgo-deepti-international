...
const EXTERNAL_ID_KEY = 'dp_external_id';
 ...
 * `window.location.search` here instead returns nothing on /register, because
 * the visitor arrived by clicking a link and the query string did not come
 * with them, and every paid registration would be written to the record with
 * blank UTMs.
 */
